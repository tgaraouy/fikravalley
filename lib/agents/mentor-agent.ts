...
export class MentorAgent {
  ...
  private explainMatch(mentor: Mentor, attributes: any): string[] {
    ...
    if (mentor.livedExperience.founded.length > 0) {
      reasons.push(`A fondé: ${mentor.livedExperience.founded[0]}`);
    }
    ...
    if (mentor.livedExperience.projects.length > 0) {
      ...
      if (relevantProject) {
        reasons.push(`A travaillé sur: ${relevantProject}`);
      }
    }
    
    // Location reason
    if (mentor.locations.includes(attributes.location)) {
      reasons.push(`Basé à ${attributes.location} (comme vous)`);
    }
    
    // Track record
    if (mentor.successRate >= 0.5) {
      reasons.push(`${Math.round(mentor.successRate * 100)}% de ses mentorés ont été financés`);
    }
    
    // Intimacy reason (Locke!)
    if (mentor.intimacyRating >= 8) {
      reasons.push(`⭐ Connaissance INTIME du domaine (${mentor.intimacyRating}/10 - Locke's standard)`);
    }
...
${connectionPoints.length > 0 ? `J'ai remarqué que ${connectionPoints[0].toLowerCase()}, ce qui me fait penser que vous avez une compréhension INTIME des défis que je rencontre.` : ''}

${mentor.livedExperience.founded.length > 0 ? `Votre expérience en tant que fondateur de ${mentor.livedExperience.founded[0]} est particulièrement inspirante. ` : ''}Ce n'est pas juste quelqu'un qui "connaît DE" ${idea.problem.sector} que je cherche, mais quelqu'un qui l'a VÉCU - et c'est clairement votre cas.

Seriez-vous disponible pour un appel de 30 minutes? J'aimerais vous présenter mon projet et bénéficier de vos conseils basés sur votre expérience réelle.
...
${creator.name}`;
    ...
    return {
      ...
      body,
      tone: 'casual',
      suggestedAsk: 'Appel de 30 minutes pour présenter le projet et obtenir des conseils',
      intimacyNote
    };
  }
  ...
  private generateIntimacyNote(mentor: Mentor, idea: IdeaStatement): string {
    if (mentor.livedExperience.founded.length > 0) {
      return `💡 Locke's Insight:

${mentor.name} a FONDÉ ${mentor.livedExperience.founded[0]}.

Ce n'est pas quelqu'un qui "connaît DE" ${idea.problem.sector}.
...
${mentor.name} a travaillé sur des projets similaires au vôtre.

${mentor.intimacyRating}/10 d'intimité avec le domaine.

Cette personne a fait les erreurs, trouvé les solutions.
Apprenez de leur vécu, pas juste de leur théorie.`;
    }
    
    return `💡 Locke's Insight:

${mentor.name} a ${mentor.livedExperience.yearsExperience} ans d'expérience dans le domaine.

Cette connaissance vient de l'ACTION, pas des livres.
Écoutez avec attention. Posez des questions sur leur VÉCU.`;
  }
  ...
  private findConnectionPoints(idea: IdeaStatement, mentor: Mentor): string[] {
    ...
    if (mentor.locations.includes(idea.problem.location)) {
      points.push(`Vous êtes tous deux à ${idea.problem.location}`);
    }
    
    // Similar past projects
    if (mentor.livedExperience.projects.length > 0) {
      points.push(`${mentor.name} a travaillé sur des projets similaires`);
    }
