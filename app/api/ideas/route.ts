...
import { autoCategorizeIdea } from '@/lib/ai/auto-categorize-idea';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Auto-categorize the idea (non-blocking, runs in background)
    try {
      const categorization = await autoCategorizeIdea({
        problem_statement: sanitizedBody.problem_statement,
        proposed_solution: sanitizedBody.proposed_solution,
        category: body.category,
        location: body.location,
        estimated_cost: body.estimated_cost || null,
        ai_capabilities_needed: body.ai_capabilities_needed || null,
        integration_points: body.integration_points || null,
        useAIFallback: true,
      });

      // Update the idea with categorization (non-blocking, don't fail if this errors)
      await (supabase as any)
        .from('marrai_ideas')
        .update({
          moroccan_priorities: categorization.moroccan_priorities.length > 0 ? categorization.moroccan_priorities : null,
          budget_tier: categorization.budget_tier,
          location_type: categorization.location_type,
          complexity: categorization.complexity,
          sdg_alignment: categorization.sdg_alignment,
        })
        .eq('id', insertedIdea.id)
        .then(({ error }: any) => {
          if (error && process.env.NODE_ENV === 'development') {
            console.error('Error updating idea categorization:', error);
          }
        });
    } catch (categorizationError) {
      // Don't fail the request if categorization fails
      if (process.env.NODE_ENV === 'development') {
        console.error('Error in auto-categorization:', categorizationError);
      }
    }
